  ...
  return (
    <div className="pt-24 pb-12">
      ...
      <section className="container mx-auto px-4 py-12 md:py-24 grid md:grid-cols-2 gap-12 items-center">
        <div>
          <span className="inline-block bg-dame-beige/30 text-dame-orange font-bold px-4 py-1 rounded-full text-sm mb-6 uppercase tracking-widest border border-dame-orange/20">
            Offre Exclusive Anniversaire
          </span>
          <h1 className="text-4xl md:text-6xl font-bold text-dame-brown leading-tight mb-6 italic">
            Fêtez votre Anniversaire comme une <span className="text-dame-orange">VIP</span> : Le Festin qui vous met à l'Honneur !
          </h1>
          <p className="text-lg md:text-xl text-dame-brown/80 mb-8 leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button 
              onClick={() => onNext({})}
              className="bg-dame-orange text-white text-lg font-bold px-8 py-4 rounded-xl shadow-xl shadow-dame-orange/30 cta-button text-center"
            >
            ...
            <div className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {[1, 2, 3, 4].map(i => (
                  <img key={i} src={`https://picsum.photos/seed/${i+40}/100/100`} className="w-10 h-10 rounded-full border-2 border-white" alt="Avatar" />
                ))}
              </div>
        ...
        <div className="relative">
          <div className="absolute -top-10 -right-10 w-40 h-40 bg-dame-beige rounded-full blur-3xl opacity-50 -z-10"></div>
          <img 
          ...
          />
          <div className="absolute bottom-6 -left-6 bg-white p-4 rounded-2xl shadow-xl border border-dame-beige">
          ...
          <div className="grid md:grid-cols-3 gap-8 text-left">
            {benefits.slice(0, 3).map((b, i) => (
              <div key={i} className="bg-white/5 p-6 rounded-2xl border border-white/10">
                <div className="w-10 h-10 bg-dame-orange rounded-lg flex items-center justify-center mb-4">
                  <CheckIcon />
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-4">
              {menuItems.map((item, i) => (
                <div key={i} className="flex items-center gap-4 p-4 rounded-xl hover:bg-dame-beige/10 transition-colors border border-transparent hover:border-dame-beige/20">
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-dame-beige/20 flex items-center justify-center text-dame-orange">
                    {i + 1}
                  </div>
            ...
            <div className="bg-dame-white rounded-3xl p-8 border-2 border-dame-orange shadow-2xl relative">
              <div className="absolute top-0 right-0 bg-dame-orange text-white px-6 py-2 rounded-bl-3xl rounded-tr-3xl font-bold animate-pulse">
                -23% MAINTENANT
              </div>
              ...
              <div className="bg-dame-brown text-white p-4 rounded-xl text-center mb-8">
              ...
              <button 
                onClick={() => onNext({})}
                className="w-full bg-dame-orange text-white py-5 rounded-2xl font-bold text-xl shadow-lg shadow-dame-orange/40 hover:scale-105 transition-all mb-4"
              >
                Réserver mon Pack VIP maintenant
              </button>
              <p className="text-center text-xs text-dame-brown/60">Garantie satisfaction : Délicieux ou Remboursé !</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <div className="space-y-4">
            {[
              ...
              <details key={i} className="group bg-white p-6 rounded-2xl shadow-sm border border-dame-beige/30 cursor-pointer">
                <summary className="font-bold text-lg flex justify-between items-center list-none">
                  {item.q}
                  <span className="text-dame-orange group-open:rotate-180 transition-transform">↓</span>
                </summary>
